...
- **gocv `cannot find -lopencv_*`** or **`opencv2/opencv.hpp: No such file or directory`** during `make build` — OpenCV isn't installed inside MSYS2 MINGW64 (or `pkg-config` can't see it). Run: `pacman -S --needed mingw-w64-x86_64-opencv mingw-w64-x86_64-pkgconf` inside the MSYS2 MINGW64 shell, then re-run `make build`. The Makefile now aborts early with this exact instruction if OpenCV is missing.
