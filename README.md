...
- **CMake error during `./scripts/install-tflite.sh`: `Compatibility with CMake < 3.5 has been removed`** (typically in `FP16-source/CMakeLists.txt`) — modern CMake (4.x, the version MSYS2 ships now) refuses TensorFlow v2.16.1's older third-party deps. The install script already passes `-DCMAKE_POLICY_VERSION_MINIMUM=3.5` to work around this; if you cloned before that fix, run `git pull` and re-run `./scripts/install-tflite.sh`. If it still fails, wipe `_third_party/tflite-build/out/` first (`rm -rf _third_party/tflite-build/out`) so CMake reconfigures from scratch.
